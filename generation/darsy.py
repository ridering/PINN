...
class DarcyFlowDataset(Dataset):
    def __init__(
        ...
        for i in trange(num_samples):
            ...
            a[i] += 1e-3
            u = np.zeros_like(a[i])
            data[i], _ = sor(u, a[i], f, 1.3, 1e-8)

        self._config['a'] = torch.tensor(a, dtype=torch.float32, device=device)
        self._config['f'] = torch.tensor(f, dtype=torch.float32, device=device)
